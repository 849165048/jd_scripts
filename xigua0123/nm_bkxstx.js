...
APP:必看小说
变量 如
bkxshdtx='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
let bkxshdtxArr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshdtx= $.isNode() ? (process.env.bkxshdtx ? process.env.bkxshdtx : "") : ($.getdata('bkxshdtx') ? $.getdata('bkxshdtx') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshdtxArr.push($.getdata('bkxshdtx'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshdtxArr.push($.getdata(`bkxshdtx${i}`))
            }
    console.log(`------------- 共${bkxshdtxArr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshdtxArr.length; i++) {
        if (bkxshdtxArr[i]) {
          bkxshdtx = bkxshdtxArr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshdtx && process.env.bkxshdtx.indexOf('@') > -1) {
            bkxshdtxArr = process.env.bkxshdtx.split('@');
            console.log(`您选择的是用"@"隔开\n`)
        } else {
            bkxshdtxs = [process.env.bkxshdtx]
        };
        Object.keys(bkxshdtxs).forEach((item) => {
        if (bkxshdtxs[item]) {
            bkxshdtxArr.push(bkxshdtxs[item])
        }
    })
          console.log(`共${bkxshdtxArr.length}个cookie`)
	        for (let k = 0; k < bkxshdtxArr.length; k++) {
                $.message = ""
                bkxshdtx = bkxshdtxArr[k]
                $.index = k + 1;
